...
def get_backbone(args, pretrained=False):
    ...
    if name == "pretrained_vit_b16_224" or name == "vit_base_patch16_224":
        ...
        if args["model_name"] == "prop":
            from backbone import vit_prop
            ...
            if name == "vit_base_patch16_224_prop":
                model = vit_prop.vit_base_patch16_224_prop(num_classes=0,
                                                           global_pool=False, drop_path_rate=0.0,
                                                           tuning_config=tuning_config)
                model.out_dim = 768
            elif name == "vit_base_patch16_224_in21k_prop":
                # model = vit_prop.vit_base_patch16_224_in21k_prop(Prompt_Token_num=args["prompt_token_num"])
                model = vit_prop.vit_base_patch16_224_in21k_prop(num_classes=0,
...
class PropNet(BaseNet):
    ...
    def extract_vector(self, x, index_tsne):
        #return self.backbone.forward_proto(x, index_tsne)
        return self.backbone(x)
