...
from utils.inc_net import PropNet
...
class Learner(BaseLearner):
    def __init__(self, args):
        super().__init__(args)
        self._network = PropNet(args, True)
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                if self.args['prompt_orth']:
                    reg_loss = 0.0
                    for i in range(len(self._network.backbone.blocks)):
                        prompt_module = self._network.backbone.cur_adapter[i]
                        reg_loss = prompt_module.regularization_loss(lambda_l2=0.01) + reg_loss
                    loss = self.args['orthrate'] * reg_loss + loss
